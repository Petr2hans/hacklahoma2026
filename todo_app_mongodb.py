...
# Task breakdown function - REAL Gemini integration
def breakdown_task(task_title, user_id):
    """
    Calls your actual Gemini breakdown from workers_breakdown.py
    """
    from workers_breakdown import breakdown_one_task
    
    # Create a temporary document structure for breakdown
    doc = {
        'task': task_title,
        'userId': user_id,
        'needsBreakdown': True
    ...
    # Call your actual Gemini breakdown function
        ...
        if self.path == '/api/register':
            ...
            try:
                ...
                try:
                    ...
                    # Sync wrapper for async Solana call
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    tx_hash = loop.run_until_complete(send_study_reward(wallet_address, credits))
                    loop.close()
                    ...
                    if tx_hash:
                        ...
                        credit_transfers_collection.update_one(
                            ...
                            {'$set': {
                                'status': 'failed',
                                'error': 'Solana transfer returned None',
                    ...
                    print(f"❌ Solana transfer error: {sol_error}")
                    
                    # Mark as error but still save record
                    credit_transfers_collection.update_one(
                        {'_id': transfer_id},
                        {'$set': {
                            'status': 'error',
                    ...
                    self.wfile.write(json.dumps({
                        'success': False,
                        'message': f'Transfer error: {str(sol_error)}'
                    }).encode())
