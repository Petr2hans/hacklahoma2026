...
RAW_TREASURY_BYTES_STR = os.environ.get('sol_key', '[]')
DECIMALS = 9
...
async def send_study_reward(user_wallet_address: str, amount: float):
    ...
    """
    try:
        # Parse treasury bytes from environment
        import ast
        raw_bytes = ast.literal_eval(RAW_TREASURY_BYTES_STR)
        
        async with AsyncClient(RPC_URL) as client:
            # 1. Setup Identities
            treasury = Keypair.from_bytes(bytes(raw_bytes))
